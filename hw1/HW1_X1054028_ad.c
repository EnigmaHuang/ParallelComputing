...
float* read_file(MPI_File *in, const int head, const int tail){
...

      {
          ...
          chunk = malloc((long long)(cur_size) * sizeof(float));
...
int main(int argc, char** argv){
    ...
    if (n <= 1){
        ...
        while (1){
            ...
            float *buffer = (float*)malloc((long long)buffer_size * sizeof(float));
            ...
            buffer = (float*)malloc((long long)buffer_size * sizeof(float));
            pbuffer = buffer;
            if (rank % 2 == 0 && rank > 0){
                MPI_Recv(buffer, block_len[rank - 1], MPI_FLOAT, rank - 1,
                    MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        cur_front_n = block_len[rank] / 2;
        cur_back_n = block_len[rank] - cur_front_n;
        qsort(chunk, cur_front_n, sizeof(float), comp);
        qsort(chunk + cur_front_n, cur_back_n, sizeof(float), comp);
        ...
        while (1){
            res_message[0] = 0;
            /* odd phrase */
            res_message[0] = merge_sort(cur_front_n, chunk, cur_back_n, chunk + cur_front_n);
            MPI_Barrier(MPI_COMM_WORLD);
